...
# Convert GeoDataFrame to GeoJSON-like format for Altair
def geojson_to_altair(gdf):
...
zip_geojson = geojson_to_altair(nyc_zips)
zip_data = nyc_zips[['postalCode', 'complaints_count']].copy()

# Create the choropleth map
choropleth = alt.Chart(alt.Data(values=zip_geojson['features'])).mark_geoshape(
    stroke='white',
    strokeWidth=0.5
).encode(
    color=alt.Color(
        'complaints_count:Q',
        title='Number of Illegal Pet Complaints',
    ...
    width=800,
    height=600,
    title='Illegal Pet Complaints in NYC by ZIP Code'
).configure_view(
    strokeWidth=0
)
